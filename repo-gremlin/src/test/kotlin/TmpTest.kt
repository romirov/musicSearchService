...
//        println("CONTENT: ${x}")
    ...
    fun repoTest() = runTest {
        val initObj = MssTopic(
            title = "x",
            description = "y",
            ownerId = MssUserId("123"),
        )
        val repo = TopicRepoGremlin(hosts = "localhost")
        val resAd = repo.createTopic(DbTopicRequest(topic = initObj))
        val ad = resAd.data ?: fail("Empty object")
        val adRead = repo.readAd(DbTopicIdRequest(ad.id))
        println("adRead: $adRead")

        val adDel = repo.deleteAd(DbTopicIdRequest(ad.id, ad.lock))
        println("adDel: $adDel")
        val adDeleted = repo.readAd(DbTopicIdRequest(ad.id))
